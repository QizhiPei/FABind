...
for epoch in range(args.sample_size):
    ...
    for batch_id, data in enumerate(data_iter):
        try:
            ...
            with torch.no_grad():
                ...
                com_coord_pred, compound_batch, confidence_score_pred = model.inference(data)
                confidence_score_list.extend(confidence_score_pred)
    ...
    if args.write_mol_to_file:
        for i in tqdm(range(len(uid_list))):
            ...
            sdf_output_path = os.path.join(args.sdf_output_path_post_optim, uid_list[i], f"{uid_list[i]}_{confidence_score_list[i].item():.4f}.sdf")
            if not os.path.exists(os.path.join(args.sdf_output_path_post_optim, uid_list[i])):
                os.makedirs(os.path.join(args.sdf_output_path_post_optim, uid_list[i]))
...
if args.write_mol_to_file:
    for i in tqdm(range(len(uid_list))):
        sdf_files = glob.glob(f"{args.sdf_output_path_post_optim}/{uid_list[i]}/*.sdf")
        numbered_files = [(f, float(f.split('/')[-1].split('.sdf')[0].split('_')[1])) for f in sdf_files]
        numbered_files.sort(key=lambda x: x[1], reverse=True)
        highest_number_file = numbered_files[0][0]
        os.system("cp {} {}".format(highest_number_file, f"{args.sdf_output_path_post_optim}/{uid_list[i]}.sdf"))
